...
class events(Extension):
    ...
    async def on_member_role_remove(self, event: MemberUpdate):
        if event.guild_id == 922523614828433419:
            ...
            roles_list = [
                role.mention for role in before_roles if role not in after_roles
            ]
            if roles_list != []:
                roles = ""
    ...
    async def on_member_role_add(self, event: MemberUpdate):
        if event.guild_id == 922523614828433419:
            ...
            roles_list = [
                role.mention for role in after_roles if role not in before_roles
            ]
            if roles_list != []:
                roles = ""
                for role in roles_list:
            ...
            if before.display_name != after.display_name:
                ...
                if stats is not None:
                    if after.display_name is None:
                        ...
                        levelling.update_one(
                            {"id": before.id},
                            {"$set": {"displayname": after.display_name}},
                        )
                embed = Embed(
                ...
                embed.add_field(
                    name="After:", value=f"`{after.display_name}`", inline=True
                )
                embed.set_author(
                    name=f"{member}",
                    icon_url=member.avatar.url,
    ...
    async def on_member_update_timeout_remove(self, event: MemberUpdate):
        if event.guild_id == 922523614828433419:
            ...
            if member_after.communication_disabled_until is None == True:
                audit_log_entry = await event.guild.fetch_audit_log(
                    action_type=24, limit=1
                )
    ...
    async def on_member_update_timeout_add(self, event: MemberUpdate):
        if event.guild_id == 922523614828433419:
            ...
            if member_after.communication_disabled_until is not None:
                timeout_timestamp = (
                    f"{member_after.communication_disabled_until}".replace("<t:", "")
                )
                ...
                if target.id == member_after.id:
                    if (member_after.communication_disabled_until is not None) and (
                        ...
                        mute_time = (
                            f"{member_after.communication_disabled_until}".replace(
                                ">", ":R>"
                            )
                        )
                        if reason is None or "":
    ...
    async def on_member_kick(self, event: MemberRemove):
        if event.guild_id == 922523614828433419:
            member = event.member
            audit_log_entry = await member.guild.fetch_audit_log(
                action_type=20, limit=1
            )
            for au_entry in audit_log_entry.entries:
                entry_created_at = snowflake_time(au_entry.id)
                cdiff = date_diff_in_Seconds(
            ...
            if not member.bot:
                ...
                if welcome_channel_id is not None:
                    ...
                    if wchannel is not None:
                        ...
                        def round(im):
                            im = im.resize(
                                (210 * 16, 210 * 16), resample=Image.ANTIALIAS
                            )
                        ...
                        overlay = Image.open("assets/welcome/mockup.png").convert(
                            "RGBA"
                        )
    ...
    @listen()
    async def on_server_boost(self, event: MemberUpdate):
        member = event.after
        server = self.bot.get_guild(922523614828433419)
        premium_role = await server.fetch_role(922896058298822676)  # type: ignore
        if not premium_role:
            return

        if member.has_role(premium_role):
            embed = Embed(
                color=0x7289DA,
                description=f"<:booster:914737485924409405> **{member.display_name}**, Terima Kasih sudah mem-boost server ini! :pray:",
            )
            embed.set_thumbnail(url=member.avatar.url)
            embed.add_field(
                name="Boosted Since:",
